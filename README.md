...
Every time we produce a jar or promote a jar to a different stage, we need to label differently. For final releases (a.k.a. public releases) we will use the versioning scheme *major*.*minor*.*patch*. For release candidate we will use *major*.*minor*.*patch*.rc.*release-candidate*. For builds we will use *major*.*minor*.*patch*.rc.*release-candidate*+*git_ref*, i.e. every git commit has a reference and we use that reference to version our jar.
